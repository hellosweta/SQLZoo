...
def heart_tracks
  ...
  execute(<<-SQL)
  SELECT
    albums.title, COUNT(tracks.song) AS heart_count
  FROM
    albums
    JOIN tracks ON albums.asin = tracks.album
  WHERE
...
def best_value
  ...
  execute(<<-SQL)
  SELECT
    albums.title, albums.price, COUNT(*) AS number_of_tracks
  FROM
    albums
    JOIN tracks ON albums.asin = tracks.album
  -- WHERE
...
def top_track_counts
  ...
  execute(<<-SQL)
  SELECT
    albums.title, COUNT(*) AS number_of_tracks
  FROM
    albums
    JOIN tracks ON albums.asin = tracks.album
  -- WHERE
...
def rock_superstars
  ...
  execute(<<-SQL)
  SELECT
    DISTINCT albums.artist, COUNT(DISTINCT albums.title) AS number_of_albums
  FROM
    albums
    -- JOIN tracks ON albums.asin = tracks.album
